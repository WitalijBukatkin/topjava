...
public abstract class AbstractJdbcMealRepository<T> implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        MapSqlParameterSource map = new MapSqlParameterSource()
                ...
                .addValue("date_time", localDateTimeConverter(meal.getDateTime()))
    ...
    protected abstract T localDateTimeConverter(LocalDateTime source);

    @Override
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query(
                "SELECT * FROM meals WHERE user_id=?  AND date_time BETWEEN  ? AND ? ORDER BY date_time DESC",
                ROW_MAPPER, userId, localDateTimeConverter(startDate), localDateTimeConverter(endDate));
    }
